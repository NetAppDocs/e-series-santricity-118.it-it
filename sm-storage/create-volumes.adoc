...
. Per continuare la sequenza di creazione del volume per l'host o il cluster host selezionato, fare clic su *Avanti* e andare a <<Fase 2: Selezionare un carico di lavoro per un volume>>.
+
...
. Fare clic su *Avanti*.
. Se il carico di lavoro è associato a un tipo di applicazione supportato, immettere le informazioni richieste; in caso contrario, andare a <<Fase 3: Aggiunta o modifica di volumi>>.
...
* Eseguire l'utilità specifica del sistema operativo (disponibile da un fornitore di terze parti), quindi eseguire il comando SMcli `-identifyDevices` per correlare i nomi dei volumi ai nomi degli array di storage host.
+
