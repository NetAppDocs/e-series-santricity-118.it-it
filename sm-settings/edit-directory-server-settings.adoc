...
URL server
 a| 
L'URL per accedere al server LDAP sotto forma di `ldap[s]://host:port` .
...
Ricerca DN base
 a| 
Contesto LDAP per la ricerca degli utenti, generalmente sotto forma di `CN=Users, DC=cpoc, DC=local` .
...
DN gruppo
 a| 
Il nome di dominio del gruppo di utenti LDAP da mappare. Sono supportate le espressioni regolari. Questi caratteri speciali di espressione regolare devono essere escape con una barra rovesciata (`\`) se non fanno parte di un modello di espressione regolare: \.[]{}()<>*+-=!?^$|
