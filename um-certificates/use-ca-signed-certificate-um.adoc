...
. Fare clic su *fine*.
. Andare a <<Fase 2: Inviare il file CSR>>.
...
* Se la CA ha fornito un file di certificato concatenato (ad esempio, un file .p7b), è necessario decomprimere il file concatenato in singoli file: Il certificato root, uno o più certificati intermedi e il certificato del server. È possibile utilizzare l'utilità Windows `certmgr` per decomprimere i file (fare clic con il pulsante destro del mouse e selezionare menu:tutte le attività[Esporta]). Si consiglia la codifica base-64. Una volta completate le esportazioni, viene visualizzato un file CER per ciascun file di certificato nella catena.
